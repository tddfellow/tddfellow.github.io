...
- [ ] Add preface and description
- [ ] Add bottom line and thanks
...
Buggy `if`-riddled code is what we've got. It is even not so easy to read. While we can refactor it to be more readable that won't change presence of bugs though. Let's still do it to understand what happens in this code better:
...
Oh! Now a bunch of other tests fail:
...
OrderKindValidator
  ...
  fails with message "Order kind can not be 'private' and 'corporate' at the same time"
    when order kind is ["private", "corporate"] (FAILED - 4)

  fails with message "Order kind can be one of: 'private', 'corporate', 'bundle'"
...
And this is why it was hard to change order of validations. We will have to completely rewrite this function. Let's start small and see which tests fail:
...
After refactoring `empty?` function now is looking this way:
...
And all tests, finally, pass. It took a lot of effort and re-writing to get this one little test to pass. This is what we call "Getting Stuck" in TDD. There is always an order of tests that will lead to this result almost for any somewhat complex problem. Trick to not get stuck is to:

- never write test that will not fail (wait until later, when it will fail), and
- always first write the test that will point out next deficiency in the current production code.

In TDD this is called Triangulation.
